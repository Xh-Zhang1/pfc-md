...
def get_perf_rule_dm_vector(output, target_choice, epochs, fix_strength=0.2, action_threshold=0.5, response_duration=int(300/20)):
    '''
    output (75, 512, 4)
    '''

    fix_start = epochs['cue_stimulus'][0]
    fix_end = epochs['stimulus'][1]

    response_on = epochs['response'][0][0]
    response_off = epochs['response'][1][0]

    batch_size = output.shape[1]#512
    action_at_fix = np.array([np.mean(output[fix_start[i]:fix_end[i], i, :]) > fix_strength for i in range(batch_size)])
...
class Trainer(object):
    def __init__(self, rule_trains=None, model=None, hp=None, model_dir=None, is_cuda=True):
        ...
        # if model is None:
        #     self.model = network.RNN(hp, is_cuda)
    ...
    def do_eval(self):
        ...
        for context_name in self.hp['rule_trains']:
            ...
            if context_name=='RDM_task':
                ...
                target_choice =   1*((sample_batched['strength_cue']*sample_batched['context_RDM']> sample_batched['batch_0'])*(np.abs(sample_batched['mean_vis_0']))> np.abs(sample_batched['mean_vis_1'])) \
                                 +2*((sample_batched['strength_cue']*sample_batched['context_RDM']> sample_batched['batch_0'])*(np.abs(sample_batched['mean_vis_0'])<=np.abs(sample_batched['mean_vis_1']))) \
                                 +3*((sample_batched['strength_cue']*sample_batched['context_RDM']<=sample_batched['batch_0'])*(np.abs(sample_batched['mean_aud_0'])> np.abs(sample_batched['mean_aud_1']))) \
                                 +4*((sample_batched['strength_cue']*sample_batched['context_RDM']<=sample_batched['batch_0'])*(np.abs(sample_batched['mean_aud_0'])<=np.abs(sample_batched['mean_aud_1'])))
            ...
            if context_name=='HL_task':
                ...
                target_choice =   1*((sample_batched['strength_cue']*sample_batched['context_HL']> sample_batched['batch_0'])*(np.abs(sample_batched['mean_vis_0']))> np.abs(sample_batched['mean_vis_1'])) \
                                 +2*((sample_batched['strength_cue']*sample_batched['context_HL']> sample_batched['batch_0'])*(np.abs(sample_batched['mean_vis_0'])<=np.abs(sample_batched['mean_vis_1']))) \
                                 +3*((sample_batched['strength_cue']*sample_batched['context_HL']<=sample_batched['batch_0'])*(np.abs(sample_batched['mean_aud_0'])> np.abs(sample_batched['mean_aud_1']))) \
                                 +4*((sample_batched['strength_cue']*sample_batched['context_HL']<=sample_batched['batch_0'])*(np.abs(sample_batched['mean_aud_0'])<=np.abs(sample_batched['mean_aud_1'])))
        ...
        success_action_prob = (success_action_prob_two[0]+success_action_prob_two[1])/2
        mean_choice_error   = (mean_choice_error_two[0]+mean_choice_error_two[1])/2
        if clsq_tmp < self.min_cost:
            self.min_cost = clsq_tmp
            self.model.save(self.model_dir)
        print('save model:',self.model_dir)
        ...
        # save model routinely
        if 0<=self.hp['stim_std']<=0.01:
            if success_action_prob>0.9:
        ...
        if 0.01<self.hp['stim_std']<=0.1:
            if success_action_prob>0.87:
                self.model_save_idx = self.model_save_idx + 1
                tools.mkdir_p(routine_save_path)
                self.model.save(routine_save_path)
                self.log['save_performance'].append(success_action_prob)
        if 0.1<self.hp['stim_std']<=0.2:
            if success_action_prob>0.8:
                self.model_save_idx = self.model_save_idx + 1
                tools.mkdir_p(routine_save_path)
                self.model.save(routine_save_path)
                self.log['save_performance'].append(success_action_prob)
        if 0.2<self.hp['stim_std']<=0.3:
            if success_action_prob>0.7:
                self.model_save_idx = self.model_save_idx + 1
                tools.mkdir_p(routine_save_path)
                self.model.save(routine_save_path)
                self.log['save_performance'].append(success_action_prob)
    ...
    def train(self, max_samples=1e5, display_step=500,max_model_save_idx=20):
        """Train the network.
        """
        ...
        for step in range(10000000):
            ...
            if step % display_step == 0:
                ...
                print('{:8d}'.format(step) +'==========================='+'  | Time ',elapsed_time)
                ...
                if success_action_prob > 0.99 and mean_choice_error<0.2:
