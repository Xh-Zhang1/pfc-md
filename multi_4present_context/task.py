...
def _RDM_task(config, mode,**kwargs):
    ...
    # print('**** cue_delay_time',cue_delay_time)
    ...
    if mode == 'random':  # Randomly generate parameters
        ...
        c_cue = rng.choice([-1.0*scale_RDM, 1.0**scale_RDM], (batch_size,))
        ...
        c_cue = rng.choice([-1.0*scale_RDM, 1.0*scale_RDM], (batch_size,))#rng.uniform(-0.06, 0.06, batch_size)
        p_coh = rng.choice([p_cohs])#
        ...
        gamma_bar_vis = rng.uniform(1.0, 1.0,batch_size)#kwargs['gamma_bar']
        gamma_bar_aud = rng.uniform(-1.0, -1.0,batch_size)#kwargs['gamma_bar']
        ...
        mean_aud_0 = gamma_bar_aud+c_aud
        mean_aud_1 = gamma_bar_aud-c_aud
        vis_modality_stim_0 = mean_vis_0#rng.normal(loc=mean_vis_0, scale=config['stim_std_test'], size=batch_size)
        vis_modality_stim_1 = mean_vis_1#rng.normal(loc=mean_vis_1, scale=config['stim_std_test'], size=batch_size)
        aud_modality_stim_0 = mean_aud_0#rng.normal(loc=mean_aud_0, scale=config['stim_std_test'], size=batch_size)
        aud_modality_stim_1 = mean_aud_1#rng.normal(loc=mean_aud_1, scale=config['stim_std_test'], size=batch_size)

    elif mode == 'test_for_pca':
        p_coh = kwargs['p_coh']
        c_cue = kwargs['c_cue']
        c_vis = kwargs['c_vis']
        c_aud = kwargs['c_aud']
        ...
        p_coh = 0#kwargs['p_coh']
        c_cue = 0#kwargs['c_cue']
        c_vis = 0#kwargs['c_vis']
        c_aud = 0#kwargs['c_aud']


        gamma_bar_vis = rng.uniform(0, 0,batch_size)#kwargs['gamma_bar']
        gamma_bar_aud = rng.uniform(0, 0,batch_size)#kwargs['gamma_bar']
        ...
        vis_modality_stim_0 = rng.normal(loc=mean_vis_0, scale=0, size=batch_size)
        vis_modality_stim_1 = rng.normal(loc=mean_vis_1, scale=0, size=batch_size)
        aud_modality_stim_0 = rng.normal(loc=mean_aud_0, scale=0, size=batch_size)
        aud_modality_stim_1 = rng.normal(loc=mean_aud_1, scale=0, size=batch_size)
    ...
    ############################## cue input
    cue_dim = config['n_ring_RDM']
    for i in range(int(cue_dim*p_coh)):
        trial.add('input', i, ons=cue_on, offs=cue_off, strengths=c_cue)

    ############################## cue input
    context_RDM=1
    context_HL=1
    trial.add('input', -6, ons=stim1_on, offs=stim1_off, strengths=vis_modality_stim_0)
    trial.add('input', -5, ons=stim1_on, offs=stim1_off, strengths=vis_modality_stim_1)
    trial.add('input', -4, ons=stim1_on, offs=stim1_off, strengths=aud_modality_stim_0)
    trial.add('input', -3, ons=stim1_on, offs=stim1_off, strengths=aud_modality_stim_1)

    trial.add('input', -2, ons=cue_on, offs=response_off, strengths=trial.expand(1))
    if mode == 'for_SNR':
        trial.add('input', -2, ons=cue_on, offs=response_off, strengths=trial.expand(0))



    output_target0 = 1. * (c_cue*context_RDM > batch_0)*(mean_vis_0 > mean_vis_1)
    output_target1 = 1. * (c_cue*context_RDM > batch_0)*(mean_vis_0 <= mean_vis_1)
    output_target2 = 1. * (c_cue*context_RDM <= batch_0)*(np.abs(mean_aud_0) > np.abs(mean_aud_1))
    output_target3 = 1. * (c_cue*context_RDM <= batch_0)*(np.abs(mean_aud_0) <= np.abs(mean_aud_1))
    # print('c_cue',c_cue)
    # #print('mean',mean_vis_0,mean_vis_1,mean_aud_0,mean_aud_0)
    # print('**',vis_modality_stim_0,vis_modality_stim_1,aud_modality_stim_0,aud_modality_stim_1)
    # print('@@',output_target0,output_target1,output_target2,output_target3)
    ...
    #trial.add('out', 4, ons=response_on, offs=response_off, strengths=trial.expand(0.1*context_RDM))
    ...
    target_choice =   1*(c_cue*context_RDM> batch_0)*(mean_vis_0 > mean_vis_1) \
                      +2*(c_cue*context_RDM> batch_0)*(mean_vis_0 <= mean_vis_1) \
                      +3*(c_cue*context_RDM<=batch_0)*(np.abs(mean_aud_0) > np.abs(mean_aud_1)) \
                      +4*(c_cue*context_RDM<=batch_0)*(np.abs(mean_aud_0) <= np.abs(mean_aud_1))
...
def _HL_task(config, mode,**kwargs):
    ...
    output_target0 = 1. * (c_cue*context_HL > batch_0)*(mean_vis_0 > mean_vis_1)
    output_target1 = 1. * (c_cue*context_HL > batch_0)*(mean_vis_0 <= mean_vis_1)
    output_target2 = 1. * (c_cue*context_HL <= batch_0)*(np.abs(mean_aud_0) > np.abs(mean_aud_1))
    output_target3 = 1. * (c_cue*context_HL <= batch_0)*(np.abs(mean_aud_0) <= np.abs(mean_aud_1))
    ...
    target_choice =    1*(c_cue*context_HL > batch_0)*(mean_vis_0 > mean_vis_1) \
                      +2*(c_cue*context_HL > batch_0)*(mean_vis_0 <= mean_vis_1) \
                      +3*(c_cue*context_HL <=batch_0)*(np.abs(mean_aud_0) > np.abs(mean_aud_1)) \
                      +4*(c_cue*context_HL <=batch_0)*(np.abs(mean_aud_0) <= np.abs(mean_aud_1))
...
def generate_trials(context_name, hp, mode, noise_on=True, **kwargs):
    ...
    if context_name=='RDM_task':
        trial.add_angle_noise_vis(on=cue_on,off=cue_off,scale=config['scale_random']*scale_RDM,p_coh=p_coh)
        for batch_idx in range(batch_size):#config['batch_size_train']
            x_temp_1 = [trial.x[i_col,batch_idx,shuffle_matrix[i_col-cue_on]] for i_col in range(cue_on,cue_off)]
            x_temp[cue_on:cue_off,batch_idx,0:n_ring_RDM] = x_temp_1
            for i in range(-6,0,1):
                x_temp[:,batch_idx,i] = trial.x[:,batch_idx,i]
        trial.x = x_temp
        #################### sparsity ####################
        for batch_idx in range(batch_size):
            idxs = np.arange(cue_on,cue_off)
            np.random.shuffle(idxs)
            num_zero = int(config['sparsity_RDM']*(cue_off-cue_on))
            idx_zero=idxs[0:num_zero]
            trial.x[idx_zero,batch_idx,0:n_ring_RDM]=0
    ...
    if config['zero_input']=='yes':
        ...
        # print(trial.x.shape)
    # print('@@@@@HL_task trial.x',trial.x[:,:,:].shape,'\n', np.around(trial.x[0:,0,-3],decimals=2))
...
if __name__ == "__main__":
    ...
    for rule_name in np.array(['RDM_task']):#,'HL_task'
        ...
        if rule_name == 'RDM_task':
            ...
            print('@@@@@HL_task trial.x',trial.x[:,0,:].shape,'\n', np.around(trial.x[20:,0,:2],decimals=2))
            print('@@@@@HL_task trial.y',trial.y[:,0,:].shape,'\n', np.around(trial.y[:,0,:2],decimals=2))

            print('@@@@@HL_task trial.x',trial.x[:,0,:].shape,'\n', np.around(trial.x[20:,1,:2],decimals=2))
            print('@@@@@HL_task trial.y',trial.y[:,0,:].shape,'\n', np.around(trial.y[:,1,:2],decimals=2))
