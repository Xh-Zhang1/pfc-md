...
def mask_md_pfc_train_type3(hp,md,pc,i_size_one):
    ...
    if hp['remove_mask']=='rm_md1_VIP':
        ...
        n_md1_zero = int(n_md1*hp['md1_VIP_zero'])
        #print('n_md1_zero',n_md1_zero)
        ...
        md2_PV_zero = int(n_md2*hp['md2_PV_zero'])
...
def type3_test(hp,md,pc,i_size_one):
    ...
    if hp['add_mask']=='add_MD2_to_PC':
        MD2_PC = np.tile([hp['scale_MD_PC']*0.5]*n_MD2, (n_PC,1))

    if hp['add_mask']=='add_bi_MD1_PC':
        PC_MD1 = np.tile([2]*n_PC, (n_MD1,1))
        MD1_PC  = np.tile([2]*n_MD1, (n_PC, 1))
    if hp['add_mask']=='add_bi_MD2_PC':
        PC_MD2 = np.tile([2]*n_PC, (n_MD2,1))
        MD2_PC  = np.tile([2]*n_MD2, (n_PC, 1))

    elif hp['remove_mask']=='rm_MD2_PC':
        MD1_PC_zero = int(n_MD1*hp['percent_zero'])
        MD1_PC_1 = np.tile([0]*MD1_PC_zero, (n_PC,1))
        MD1_PC_2 = np.tile([1]*(n_MD1-MD1_PC_zero),(n_PC,1))
        MD1_PC = np.concatenate((MD1_PC_1,MD1_PC_2), axis=1)

        n_MD2_zero = int(n_MD2*hp['percent_zero'])
        #print('n_MD2_zero',n_MD2_zero)
        MD2_PC_1 = np.tile([0]*n_MD2_zero, (n_PC,1))
        MD2_PC_2 = np.tile([1]*(n_MD2-n_MD2_zero),  (n_PC,1))
        MD2_PC = np.concatenate((MD2_PC_1,MD2_PC_2), axis=1)
    ...
    elif hp['remove_mask']=='rm_MD2_VIP':
        n_MD2_zero = int(n_MD2*hp['percent_zero'])
        #print('n_MD2_zero',n_MD2_zero)
        MD2_VIP_1 = np.tile([0]*n_MD2_zero, (n_VIP,1))
        MD2_VIP_2 = np.tile([1]*(n_MD2-n_MD2_zero),  (n_VIP,1))
        MD2_VIP = np.concatenate((MD2_VIP_1,MD2_VIP_2), axis=1)

    elif hp['remove_mask']=='rm_MD1_PV':
        MD1_PV_zero = int(n_MD1*hp['percent_zero'])
        MD1_PV_1 = np.tile([0]*MD1_PV_zero, (n_PV,1))
        MD1_PV_2 = np.tile([1]*(n_MD1-MD1_PV_zero),  (n_PV,1))
        MD1_PV = np.concatenate((MD1_PV_1,MD1_PV_2), axis=1)
